...
(define (emit-if stack-index env expr)
  (let ((alt-label (unique-label "alternative"))
    ...
    (if (predicate? (if-test expr))
      ...
      (begin
        (emit-comment "if")
        (emit-expr stack-index env (if-test expr))
        (print "  cmp eax, " (immediate-rep #t))
        (emit-comment "end if")))
    (print "  jne " alt-label)
    (emit-expr stack-index env (if-consequent expr))
    (print "  jmp " end-label)
    (print alt-label ":")
    (emit-expr stack-index env (if-alternative expr))
    (print end-label ":")))

(define (emit-tail-if stack-index env expr)
  (let ((alt-label (unique-label "alternative"))
        (end-label (unique-label "end")))
    (if (predicate? (if-test expr))
      (let ((name (car (if-test expr)))
      ...
      (begin
        ...
        (emit-comment "end if")))
    (print "  jne " alt-label)
    (emit-tail-expr stack-index env (if-consequent expr))
    (print "  jmp " end-label)
    (print alt-label ":")
    (emit-tail-expr stack-index env (if-alternative expr))
    (print end-label ":")))
